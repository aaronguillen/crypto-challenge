    ...
    for i in range(0, len(cipherText)):
        for j in range(0, numCol):
            if (cipherText[i][j] != 0) and (cipherText[i][j] != "0"):
                sys.stdout.write(cipherText[i][j])
        print()
...
def printBlocked():
    ...
    for i in range(0, len(cipherText)):
        ...
        for j in range(0, numCol):
            if (cipherText[i][j] != 0) and (cipherText[i][j] != "0"):
                sys.stdout.write(cipherText[i][j] + " ")
            else: #Choose a random uppercase letter for empty spaces
                sys.stdout.write(chr(random.randint(65, 90)))
        if (i + 1) % 7 == 0:
            print()
            print("-" * ((numCol * 2) + 7))
        else:
...
def swapRows():
    ...
    for i in range(0, 3):       #Because we have 3 blocks
        for j in range(0, 7):   #of 7 rows each
            temp.append(cipherText[int((i * 7) + order[j])])
...
#Typecast numCol back to an integer
numCol = int(numCol)

#cipherText is going to be an array of all zeros and of size [numCol][21]
cipherText = [[0 for x in range(numCol)] for x in range(21)]
counter = 0
